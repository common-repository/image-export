...
define( 'IMAGE_EXPORT_VERSION', '1.1.1' );
...
function ie_execute() {
	...
	$zip_file = new PclZip( $base_dir . '/' . $download_file_name );
	$archive = $zip_file->create( $lists, PCLZIP_OPT_REMOVE_ALL_PATH );
	$file_size = file_size_format( filesize( $base_dir . '/' . $download_file_name ) );
	...
	if ( $posts ) {
		...
		unlink( $base_dir . '/' . $download_file_name );
	}
